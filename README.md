...
- The folder `models/marts/dimmensions/` is spelled as committed; rename to `dimensions/` is a trivial follow-up.
- `profiles.yml` contains placeholder Snowflake connection details. **Before the repo is shared publicly, credentials should be moved to environment variables** (e.g., `{{ env_var('SNOWFLAKE_PASSWORD') }}`). This is called out in the submission document as a follow-up.
- `uv.lock` is included because the environment was managed with `uv`; a `requirements.txt` equivalent can be produced on request.

---

## 11. Future Improvements

- Flatten the full `discounts` array rather than applying only the first discount.
- Add a near-real-time path using Snowflake Streams + Tasks for the orders source.
- Cluster `fct_orders` on `(region, order_updated_at)` once volume justifies it.
- Add dbt `source freshness` alerts wired into the scheduler.
- Move Snowflake credentials out of `profiles.yml` into environment variables.
- Add a `dim_date` for time-intelligence in Power BI.
- Introduce snapshots for slowly changing attributes on `dim_customer` (e.g., loyalty tier history).
